...
Missing_Statistics <- function(dat_file, pct = FALSE){
  pct_multiply = 1
  if (pct){
    pct_multiply <- 100
  }
  print("Missing By Case")
  per_row <- apply(dat_file, 1, function(x) sum(is.na(x)) / ncol(dat_file) * pct_multiply )
  per_row <- cbind(Case_Row_Number = which(per_row > 0), Amount_Missing = round(per_row[per_row > 0],3) )
  print(per_row)
  
  print("Missing By Item")
  per_col <- apply(dat_file, 2, function(x) sum(is.na(x)) / nrow(dat_file) * pct_multiply )
  print(round(per_col[per_col > 0],3))

  print("Missing By Overall")
  per_overall <- sum(is.na(dat_file)) / (nrow(dat_file) * ncol(dat_file)) * pct_multiply
  print(round(per_overall,3))

}


Heat_Map_Comparison <- function(x, y, main_x = "Heat Map of All Values", main_y = "Heat Map Showing Missing Values"){
  # X data is the orignal dataset
  # Y data is th dataset with the missing dataset.
  par(mfrow=c(1,2))
  image(is.na(x),col=1:2, main=main_x, axes=FALSE, xlab = "Cases", ylab = "Items" ) # if you wish to have a heat mao of how the data is spread

  image(is.na(y), col=1:2, main=main_y, axes=FALSE, xlab = "Cases", ylab = "Items") # Only the spread of misisng vlaues
